...
## 4.1 Shared single-graph policy backbone and execution-time information boundary
...
The decentralized actor receives its local observation and the legal graph representation only. Its local feature vector contains self motion state, legally available relative target information, detection state, local attack-window state, energy, role, incoming connectivity, message freshness, and target-cache confidence. In particular, an agent that neither detects the target nor receives a fresh legal cache cannot recover the true target state through the graph encoder. The actor never receives a failure label, a shortest path, future link states, or simulator ground truth. A centralized critic is used during training under centralized training with decentralized execution (CTDE), but it adds no execution-time information to the actor.

UTR and DRTP share one Single-Graph MAPPO actor--critic. Node features are linearly encoded, processed by two shared graph-attention layers, fused with the local observation representation, and mapped to logits for the 27 actions:

\[
a_{i,t}\sim\pi_\theta(a_{i,t}\mid o_{i,t},G_{i,t}).
\]

The critic estimates \(V_\phi(s_t)\) during training. Both arms use clipped PPO with generalized advantage estimation: learning rate \(3\times10^{-4}\), discount factor 0.99, GAE coefficient 0.95, clipping coefficient 0.2, entropy coefficient 0.01, value-loss coefficient 0.5, maximum gradient norm 0.5, and four PPO epochs per batch. Each actor--critic has 116,728 trainable parameters. Thus, the primary comparison changes the training distribution rather than network capacity, the PPO objective, reward, environment, or actor information.

## 4.2 Frozen relay-failure groups and uniform topology randomization
...
## 4.3 Bounded adaptive topology-perturbation reweighting
...
## 4.4 Training and deployment boundary
