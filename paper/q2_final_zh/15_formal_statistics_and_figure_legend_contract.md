...
主端点为 \(J_{F0}\)、\(J_{OOD,mean}\) 与 \(J_{OOD,worst}\)，正常工况 \(J_{nominal}\) 是能力保持端点。正式结果中，上述三个鲁棒性端点均为 5/5 正向配对差值；这支持冻结任务中的正向中心趋势，但不支持“对所有随机初始化稳定优越”或一般分布鲁棒最优性。
...
**图4：** 十个未见故障时机、持续时间和复合 OOD 条件下的平均配对任务得分差值，柱顶为正向训练种子数。
