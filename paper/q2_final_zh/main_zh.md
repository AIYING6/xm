...
> **稿件状态：** 中文结果整合稿 v1.4（主消融与外部参考分层修订）。正式训练种子 2301–2305 的共同 10M 最终检查点、12,000 条配对评估记录和训练前冻结的确认门槛均已完成审计；MAPPO-NoGraph 外部参考的五种子 10M 归档、共同评价 tape 与安全记录也已独立核验。本轮修订只完善证据层级、指标解释、方法可复现性和投稿完整度，不改写历史结论。本文只维护中文全文，不挑选中间检查点或训练种子，并永久保留历史开发 `NO-GO`、留出验证 `FAIL` 与训练种子敏感性边界；机器可读裁决仅见附录B及归档材料。
...
为补充同任务下的外部性能定位，本文还训练并评价了 MAPPO-NoGraph。该参考使用相同训练种子、10M 预算、环境、奖励和共同评价样本带，但移除了图消息输入，参数量为 35,771，与 UTR/DRTP 的 116,728 参数 SG 主干不同。因此，它只能回答该无图 MAPPO 参考在冻结任务中的性能位置，不能归因图结构、模型容量或 DRTP 自适应加权的单独作用。对 TAPE、M3DDPG 和相关无人机中继方法的审计表明，它们需要改变当前任务或学习合同，因而本文将其用于知识定位，不堆砌表面公平、实质不可比的对比方法。
...
**表2b｜MAPPO-NoGraph 外部参考结果。** 五个训练种子、最终 10M 检查点和共同 12 条件评价 tape 全部保留；参数差异意味着此表不替代表2的 UTR–DRTP 主消融。
...
本文至少存在七项边界。第一，实验限于冻结的“侦察机–中继机–攻击机”三无人机轻量 3DOF 仿真，尚无 4/5 无人机规模扩展、硬件在环（HIL）或实飞证据。第二，尽管已提供 MAPPO-NoGraph 外部参考，主因果结论仍仅来自参数匹配的内部 UTR 主消融；MAPPO-NoGraph 的消息输入和参数量不同，且其碰撞、超时优于 DRTP，因此不构成外部算法排行榜或图结构单独效应的证明。第三，本文未设置固定非均匀权重对照，因此现有结果不能单独识别在线自适应相对于静态非均匀重加权的必要性。第四，十个跨扰动条件的具体成员曾参与训练，因此它们不能作为严格未见 OOD 的 confirmatory 证据。第五，训练种子数量为五，统计结论以描述性配对效应、全体种子方向和最差配对结果为主，不支持广泛总体推断。第六，历史证据显示真实训练种子敏感性，其根因尚未建立。第七，DRTP 是有界经验重加权，不提供一般分布鲁棒或最坏情况优化保证。
...
在冻结的前瞻性五种子正式比较中，DRTP 在 F0、跨扰动均值和跨扰动最差端点上均取得正的配对均值和中位数，五个训练种子均未达到灾难性种子定义；其超时率低于 UTR，风险集中的故障触发全部有效。作为补充，MAPPO-NoGraph 外部参考的任务端点总体低于 DRTP，但在碰撞和超时上更低，进一步要求本文将任务收益与安全权衡并列报告。由于主消融未包含固定非均匀权重对照，本文结论限定为：DRTP 在该冻结任务中相对于均匀拓扑随机化显示出有前景的平均拓扑扰动鲁棒性收益，但其训练可靠性仍需被明确报告和进一步研究，且在线自适应的独立必要性尚未被单独验证；因此不将其表述为稳定、普适或对所有静态方案优越的鲁棒方法。
...
投稿前将以匿名或公开仓库方式提供以下最小可复核材料：正式训练与评估合同；环境、网络与 PPO 配置；十个 UTR/DRTP 最终 checkpoint 及 runtime-state 的 SHA256；MAPPO-NoGraph 外部归档 SHA256、五个最终 checkpoint 和 run manifest 的可核验信息；配对评估样本带清单和哈希；12,000 条主消融和 6,000 条外部参考原始 episode 指标；逐条件与逐种子汇总表；采样器日志；统计、作图和完整性审计脚本。检查点实体文件如受体积或平台限制无法直接分发，仓库仍须给出可核验哈希、获取方式和可重建说明。仓库地址、访问权限、匿名审稿与正式发布的时点，以及作者、资助和利益冲突元数据，须由作者在投稿版本中按目标期刊要求补充；在这些信息落实前，本节不构成“数据和代码已公开”的声明。
