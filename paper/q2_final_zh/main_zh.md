...
> **证据说明：** 正式主 cohort（训练种子 2301–2305）的共同 10M 最终检查点、12,000 条配对评估记录和训练前冻结确认门槛均已完成审计；无图 MAPPO 性能参考的五种子 10M 归档、共同评价 tape 与安全记录也已独立核验。另一个独立三方法 cohort（训练种子 2401–2405）完整保留为跨 cohort 可靠性重复证据，且不与正式主 cohort 合并统计。两批 cohort 的全部最终 checkpoint 另完成一次性、零训练的 12,000 条附加未见条件评价。本文不挑选中间检查点或训练种子，并保留全部历史和独立 cohort 的不利结果。

## 摘要

跨评价带的零训练诊断显示，正式主 cohort 在两张冻结评价带上均保持正向，而独立 cohort 在两张评价带上均保持反向；评价带更换不足以解释两批训练 cohort 的方向差异。进一步的一次性附加未见条件评价在六个训练支持集之外的 onset--duration 成员上重复这一 cohort 内方向差异：正式 cohort 全部正向，而独立 cohort 全部反向。

异构多无人机协同依赖由角色、感知、通信与任务支持共同形成的动态关系结构。中继节点故障并不必然造成完全信息中断；在合法直连仍然存在时，它仍可能使通信路径由中继转发重构为直接传递，并改变任务支持来源与协同几何。针对这一结构性扰动，本文构建中继故障下的拓扑鲁棒多智能体协同问题，并提出有界自适应拓扑扰动重加权单图 MAPPO（DRTP-SG-MAPPO，以下简称 DRTP）。该方法保持 116,728 参数的单图 MAPPO 策略、PPO 目标、奖励和执行信息边界不变，在固定 50% 正常工况暴露锚点的条件下，根据各拓扑扰动组相对正常工况的性能缺口，对六类故障组进行有界自适应加权。为识别训练期加权相对于均匀加权的增量作用，本文采用参数量、拓扑组、训练预算和评估协议完全匹配的均匀拓扑随机化基线 UTR-SG-MAPPO。正式主 cohort 使用五个训练前冻结的配对训练种子、统一 10M 训练预算，以及覆盖正常工况、典型故障 F0、故障时机、故障持续时间和复合扰动条件的共同配对评估样本带。以训练种子为独立统计单位，DRTP 相对 UTR 在 F0、跨扰动均值和跨扰动最差条件上的配对差值均值分别为 52.13、55.00 和 63.01，中位数分别为 35.04、33.00 和 38.40，均为 5/5 种子正向；未出现灾难性种子，训练前冻结的安全与触发有效性门槛均通过。DRTP 的平均超时率由 0.874 降至 0.694，但碰撞率由 0.005 升至 0.008。一次性附加未见条件评价中，正式 cohort 在六个训练支持集之外成员上均保持正向，而独立 cohort 在相同成员上均为负向。另一方面，独立三方法 10M cohort 中 DRTP 相对 UTR 在四个任务端点均为负向，且出现灾难性种子。因此，本文只能报告在正式主 cohort 中观察到的有界经验收益，并将跨训练 cohort 的可靠性不足作为主结论的一部分；不主张信息恢复、一般未见条件泛化、通用拓扑泛化、对随机初始化的稳定优越性，或在线自适应相对于固定非均匀权重具有已被单独证明的必要性。
