...
language: zh-CN
status: evidence-led intermediate manuscript
...
在间歇感知与受限通信下执行协同拦截的异构无人机编队，关键中继节点失效会中断任务链。仅报告终局成功率，不能说明编队在失效持续期间何时重新获得可执行的协同能力。本文研究故障后的任务链恢复时序，提出 EA-RG-MAPPO-S（简称 EA-RG）：在集中训练、分散执行框架中，将感知、环境递送通信和任务支撑表示为三类关系，并以带边特征的关系专属图注意力及联合图残差聚合这些依赖。我们在锁定的 nominal held-out 评估中，以匹配的失效暴露 episode 为对象，使用 Kaplan–Meier 曲线和受限平均生存时间（RMST）刻画恢复。3 个独立训练 seed 的结果显示：在与节点失效持续时间对应的预设 \(\tau=80\) 步窗口内，EA-RG 的 RMST 为 11.81 步，MAPPO 为 15.51 步；Full−MAPPO 的三个 seed 差异均为负，分层配对 bootstrap 95% 区间为 [−7.16, −1.05] 步。至共同完整随访上限 \(\tau=220\)，EA-RG 对 MAPPO 仍较低，但与 HAPPO 及宽单图基线的比较不呈现一致 seed 方向。受控消融表明，Gate Prior 与 Task-Support 提供受限的支持性证据，而静态 Role-Pair 调制未显示稳定的独立增益。零样本分布变化评估进一步表明，这一早期恢复优势依赖于变化类型。结果将贡献限定为：在锁定 nominal 条件下，多关系任务图与较早的故障后恢复相关，而非对所有基线或未知分布的普适优越。
...
本文的主要发现不是“图方法在所有设置下更优”，而是在锁定 nominal held-out、匹配失效暴露的条件下，三关系任务图对应于相对 MAPPO 更早的故障后恢复。\(\tau=80\) 与实际故障持续窗口对齐，使该比较回答的是故障尚未解除时任务链能否较早重建，而不是仅比较最终是否完成任务。

方法设计的解释也应保持在代码与消融能够支持的范围内。三关系编码将可感知、已递送和任务相关的依赖分开输入策略；这不同于声称学习到新的物理通信协议。Gate Prior 和 Task-Support 的移除结果支持它们作为整体设计的组成部分，但高 seed 异质性限制了机制性推断。Role-Pair 调制未显示稳定的独立收益，提示后续工作应把动态、可观测的故障相关结构与静态角色先验区分开来验证。

结果也有明确边界。完整随访下 EA-RG 与 HAPPO、宽单图不呈现统一优势；零样本 OOD 的表现随变化族而变。当前环境是 3DOF 仿真，未构成 JSBSim/6DOF、真实传感器或真实通信系统验证。因而，本文的结论应被理解为一个关于故障后恢复时序的受控模拟证据，而非部署性能或普适泛化的证明。

## 7 结论

本文以故障后任务链恢复为中心，构建了区分感知、通信和任务支撑的三关系图协同决策方法，并在锁定 nominal held-out 评估中以 KM/RMST 报告恢复时序。EA-RG 相对 MAPPO 在节点失效活动窗口内呈现一致的较早恢复，而其全时域表现与 HAPPO、宽单图仅具竞争性。消融和 OOD 结果进一步限定了组件解释与分布迁移边界。后续研究应在更高保真动力学、可观测故障条件和预先设计的跨分布协议中检验这些恢复规律。
