...
`P6R_CONDITIONAL_GO_TO_N0_ONLY`。本文件不授权环境实现、训练、云端计算或论文题目冻结。
...
P6R 是当前唯一保留的**高上限条件候选**，因为它把旧 P6 的失败转化为一个明确的、可在训练前否决的博弈结构要求；但它尚非 GO。下一步仅为 N0.1--N0.3 的零训练博弈构造审计，不能开始 MAPPO 或任何正式算法开发。
