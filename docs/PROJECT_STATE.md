...
- **TATG-MAPPO P1** completed as `TATG_P1_INFORMATION_GAP_PRESENT`. In each of two disjoint, scripted, non-learning state cohorts, the current actor-legal structural topology code (including the present edge-age proxy) mapped to more than one transition label, while a one-step legal topology history had no mixed-label code. This is a narrow topology-representation result only: no policy, reward, return, evaluation tape, checkpoint or PPO update was used. It authorizes only an exact formula/fairness/serialization audit, not TATG implementation or training. See `docs/tatg_mappo_p1_20260904/`.
