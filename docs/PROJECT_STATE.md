...
- **TATG-MAPPO C3.5** completed as `TATG_C35_ACTOR_RUNNER_INTEGRATION_PASS`. A new isolated actor adapter owns the CETM lifecycle at the runner boundary: it snapshots state before a rollout, computes action log-probability before recording the selected action, resets only completed slots from their reset graph, and replays the full stored sequence chronologically. Its three-tensor payload restores the next actor call exactly. CETM, generic snapshot-GRU and zero-residual CETM share that interface and capacity. The adapter has no environment, critic, reward, sampler, evaluation or checkpoint-selection path; C3.5 used synthetic tensors only with 0 environment steps and 0 formal PPO updates. It authorizes only a separately frozen outer rollout-loop and strict runtime-checkpoint integration audit, not training. See `docs/tatg_mappo_c35_20260904/`.
