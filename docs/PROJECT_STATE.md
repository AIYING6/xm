...
Next, decide whether the current 10-update strict-sensing result is sufficient for the manuscript or whether to run a longer strict-sensing fine-tuning budget for polish. If no longer run is needed, start drafting the 3DOF experiment section around three layers of evidence: relay-failure recovery, mechanism ablations, and strict-sensing scenario depth.
