...
**Status: FROZEN before corrected formal training.**
...
The fixed confirmatory population contains 300 episodes per method. It is split
into two prespecified analysis populations of 150 episodes each:

- **Early:** deterministic anchor episodes 0–149;
- **Nominal:** deterministic anchor episodes 150–299.

Both populations use the same frozen environment, packet/cache semantics,
failure onset, and action-policy evaluation procedure. No episode is removed
because it is difficult or because its outcome is unfavorable.
...
censoring rule cannot change after results are observed. The primary comparator
is corrected EA-RG Full versus matched-information non-graph. Corrected
wider single-graph is the secondary comparator; MAPPO/HAPPO are conditional
system-level comparators under the no-graph invariance audit.

## Analysis lock

Checkpoint selection uses only the frozen validation population and the rule in
[FAIR_ACTOR_PROTOCOL_V1_8.md](FAIR_ACTOR_PROTOCOL_V1_8.md). Confirmatory episodes
...
intervals. Report Early, Nominal, and their prespecified pooled analysis
separately; do not change the population split after inspection.
