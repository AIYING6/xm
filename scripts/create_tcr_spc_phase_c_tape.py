"""Create the frozen Phase-C development-only 440k evaluation tape."""
...
PROTOCOL = "TCR-SPC-PHASE-C-DEVELOPMENT-TAPE-V1"
TAPE_START, EPISODES = 440000, 100
CONDITIONS = {
    "nominal": None,
    "f0_seen_44_80": (44, 80),
...
FORBIDDEN = ("340000-340099", "350000-350049", "360000-360099", "370000-370049", "380000-380099", "410000-410099", "420000-420099", "430000-430099")
...
def main() -> None:
    ...
    if not args.execute:
        raise SystemExit("NO-GO: explicit --execute is required to create the Phase-C tape")
    ...
    if path.exists():
        ...
        if existing.get("tape_hash") != manifest["tape_hash"]:
            raise RuntimeError("existing Phase-C tape differs from the frozen manifest")
    else:
