#!/usr/bin/env bash
# D2-R2 method-blind CUDA calibration; run only on the frozen source commit.
set -euo pipefail

PYTHON_BIN="${PYTHON_BIN:-python}"
OUT_ROOT="${OUT_ROOT:-results/v1_9_d2_r2_budget_calibration}"
...
if not torch.cuda.is_available() or torch.cuda.get_device_properties(0).total_memory < 16 * 1024**3:
    raise SystemExit("D2-R2 requires CUDA GPU memory >=16 GiB")
...
common=(--env-name 3d_intercept --num-envs 8 --rollout-steps 128 --role-dim 8 --intent-dim 8 --ppo-epochs 4 --device cuda
  ...
  --updates 300 --eval-interval 20 --eval-episodes 16 --save-interval 20 --save-snapshots --validation-event-logging --protocol-version "$PROTOCOL")

run_one () {
  local method="$1" encoder="$2" hidden="$3" seed="$4" eval_seed="$5"
  local run_dir="$OUT_ROOT/${method}_seed${seed}"
  mkdir -p "$run_dir"
  "$PYTHON_BIN" scripts/train_ri_gmappo.py "${common[@]}" --seed "$seed" --eval-base-seed "$eval_seed" --graph-encoder "$encoder" --hidden-dim "$hidden" --method-label "$method" --run-id "v1_9_d2_r2_${method}_seed${seed}" --out-dir "$run_dir" >"$run_dir/train.stdout.log" 2>"$run_dir/train.stderr.log"
}

for seed in 9501 9502; do
  eval_seed=$((2950100 + 100 * (seed - 9500)))
...
"$PYTHON_BIN" scripts/check_v1_9_d2_r2_artifacts.py --root "$OUT_ROOT" --expected-source-commit "$EXPECTED_SOURCE_COMMIT" --output "$OUT_ROOT/D2_R2_ARTIFACT_AND_STABILITY_GATE_MANIFEST.json"
