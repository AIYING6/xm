...
foreach ($arm in @(@("full_gate","relation_conditioned"), @("no_role_gate","none"))) {
  foreach ($seed in 101,202,303) {
    ...
    $manifest = [ordered]@{ artifact_class="DEVELOPMENT_ONLY"; arm=$arm[0]; seed=$seed; git_sha=$gitSha; config_path=$config; config_sha256=(Get-FileHash -Algorithm SHA256 $config).Hash; environment_steps=200192; start_time=(Get-Date).ToUniversalTime().ToString("o"); completion_status="running"; checkpoint="actor_critic_latest.pt"; telemetry="role_gate_telemetry.csv" }
    ...
    $manifest.completion_status="completed"; $manifest.end_time=(Get-Date).ToUniversalTime().ToString("o"); $manifest.checkpoint_sha256=(Get-FileHash -Algorithm SHA256 $checkpoint).Hash
    $manifest | ConvertTo-Json | Set-Content -Encoding utf8 (Join-Path $out "run_manifest.json")
  }
}
