"""L4 message-delay development progression."""
...
OUT = ROOT / "results" / "l4_delay_development"
TRAIN_SEEDS = (8901, 8902)
EVAL_SEEDS = tuple(range(890_000, 890_032))
UPDATES = 60
HORIZON = 180
PROTOCOL = "L4_MESSAGE_DELAY_DEVELOPMENT_V1"
...
def main() -> None:
    ...
    (OUT / "L4_MANIFEST.json").write_text(json.dumps({
        "status": "L4_MESSAGE_DELAY_DEVELOPMENT",
        ...
        "only_added_complexity": "message delay 8; range scale 0.5 and dropout 0.3 retained",
        "communication_range_scale": 0.5,
        "communication_dropout_prob": 0.3, "message_delay_steps": 8, "config": asdict(template),
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    trained = []
    for seed in TRAIN_SEEDS:
        run = OUT / f"l4_delay_seed{seed}"
        run_cfg = cfg(seed, run)
        ckpt = run / "actor_critic_latest.pt"
        if not ckpt.exists():
            l0.train_ri_gmappo(run_cfg)
        trained.append((f"l4_delay_seed{seed}", run_cfg, l0.load_agent(run_cfg, ckpt)))
    rows = []
    ...
    learned = [r for r in summary if r["mode"].startswith("l4_delay")]
    ...
    if len(positive) == len(TRAIN_SEEDS):
        verdict = "L4_DELAY_LEARNING_SIGNAL_RETAINED"
    elif len(positive) == 0:
        verdict = "L4_DELAY_NO_GO__DELAY_ASSOCIATED_LEARNING_BREAKPOINT"
    else:
        verdict = "L4_DELAY_PARTIAL_DEGRADATION"
    payload = {"verdict": verdict, "summary": summary, "performance_use_prohibited": True}
    (OUT / "L4_VERDICT.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
