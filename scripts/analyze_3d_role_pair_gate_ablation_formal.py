...
DEFAULT_ABLATED = (
    ROOT
    / "results"
    / "intercept_3d_no_role_pair_gate_topology_formal_scale_matched"
    / "robustness_eval"
    / "episode_metrics.csv"
)
DEFAULT_OUT_CSV = ROOT / "results" / "intercept_3d_role_pair_gate_ablation_formal_scale_matched_summary.csv"
DEFAULT_OUT_MD = ROOT / "docs" / "intercept_3d_role_pair_gate_ablation_formal_scale_matched_summary.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze formal full-vs-no-role-pair-gate 3DOF ablation.")
...
def select_ablated(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    return [
        ...
        and row.get("graph_message_ablation", "no_role_pair_gate") == "no_role_pair_gate"
    ]
...
def build_summary(full_rows: list[dict[str, str]], ablated_rows: list[dict[str, str]], samples: int, seed: int) -> list[dict[str, str]]:
    ...
    for scenario in sorted(set(full_means) & set(ablated_means)):
        ...
        for metric in METRICS:
            ...
            row[f"no_role_pair_gate_{metric}_mean"] = f"{ablated_means[scenario][metric]:.6g}"
...
def write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    if not rows:
        raise RuntimeError("No paired formal role-pair-gate ablation rows were generated")
...
def write_md(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        lines.append(
            ...
            f"{float(row['full_success_mean']):.3f} / {float(row['no_role_pair_gate_success_mean']):.3f} | "
            f"{fmt_ci(row, 'success')} | "
            f"{float(row['full_post_failure_chain_recovered_mean']):.3f} / {float(row['no_role_pair_gate_post_failure_chain_recovered_mean']):.3f} | "
