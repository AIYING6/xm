"""L2 limited-communication-range development progression."""
...
OUT = ROOT / "results" / "l2_limited_communication_development"
TRAIN_SEEDS = (8501, 8502)
EVAL_SEEDS = tuple(range(850_000, 850_032))
UPDATES = 60
HORIZON = 180
PROTOCOL = "L2_LIMITED_COMMUNICATION_RANGE_DEVELOPMENT_V1"
...
def cfg(seed: int, out_dir: Path, updates: int = UPDATES):
    ...
    return replace(
        ...
        run_id=f"l2_limited_comm_seed{seed}",
    )
...
def main() -> None:
    ...
    (OUT / "L2_MANIFEST.json").write_text(json.dumps({
        "status": "L2_LIMITED_COMMUNICATION_RANGE_DEVELOPMENT",
        ...
        "only_added_complexity": "communication range scale 0.5; role-specific heads and non-attacker commit mask retained",
        "communication_range_scale": 0.5,
        "communication_dropout_prob": 0.0, "message_delay_steps": 0, "config": asdict(template),
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    trained = []
    for seed in TRAIN_SEEDS:
        run = OUT / f"l2_limited_comm_seed{seed}"
        run_cfg = cfg(seed, run)
        ckpt = run / "actor_critic_latest.pt"
        if not ckpt.exists():
            l0.train_ri_gmappo(run_cfg)
        trained.append((f"l2_limited_comm_seed{seed}", run_cfg, l0.load_agent(run_cfg, ckpt)))
    rows = []
    ...
    learned = [r for r in summary if r["mode"].startswith("l2_limited_comm")]
    ...
    if len(positive) == len(TRAIN_SEEDS):
        verdict = "L2_LIMITED_COMMUNICATION_LEARNING_SIGNAL_RETAINED"
    elif len(positive) == 0:
        verdict = "L2_LIMITED_COMMUNICATION_NO_GO__RANGE_CONSTRAINT_BREAKS_LEARNABILITY"
    else:
        verdict = "L2_LIMITED_COMMUNICATION_PARTIAL_DEGRADATION"
    payload = {"verdict": verdict, "summary": summary, "performance_use_prohibited": True}
    (OUT / "L2_VERDICT.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
