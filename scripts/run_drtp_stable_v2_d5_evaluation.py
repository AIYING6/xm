"""Evaluate the nine frozen D5 final-0.5M checkpoints; never train."""
...
PROTOCOL = "DRTP-STABLE-V2-D5-PILOT-EVALUATION-V1"
...
def main() -> None:
    ...
    if not args.execute or args.workers != 9:
        raise SystemExit("D5 freezes exactly 9 evaluation workers")
    ...
    print(f"D5 evaluation: workers=9, cells={len(tasks)}, episodes={total}", flush=True)
    with ProcessPoolExecutor(max_workers=9, mp_context=mp.get_context("spawn")) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(f"D5 evaluation progress {done}/{total} ({100 * done / total:.2f}%)", flush=True)
    ...
    for arm in ARMS:
        for seed in SEEDS:
            for condition in tape["conditions"]:
                ...
                if len(subset) != 100:
                    raise RuntimeError(f"incomplete D5 evaluation cell: {arm}/seed{seed}/{condition['name']}")
                summary.append({
                    "method": arm,
                    "train_seed": seed,
    ...
    with (target / "evaluation_manifest.json").open("w", encoding="utf-8", newline="\n") as handle:
