...
def plot_metric(rows: list[dict], radius: float, metric: str, ylabel: str, out_png: Path) -> None:
    ...
    for method in METHODS:
        ...
        part.sort(key=lambda row: float(row["target_speed"]))
        x = [float(row["target_speed"]) for row in part]
    ...
    ax.set_title(f"{ylabel} under radius {radius:g}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot target-speed robustness results.")
...
def main() -> None:
    ...
    for radius in radii:
        success_png = args.out_dir / f"speed_robustness_success_r{radius:g}.png"
        collision_png = args.out_dir / f"speed_robustness_collision_r{radius:g}.png"
        plot_metric(rows, radius, "success", "Success rate", success_png)
        plot_metric(rows, radius, "collision", "Collision rate", collision_png)
        print(success_png)
        print(collision_png)
