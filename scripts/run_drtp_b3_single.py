"""Execute exactly one frozen B3 UTR/DRTP trajectory (cloud only)."""
...
PROTOCOL = "DRTP-B-LINE-B3-DEVELOPMENT-V1"
SOURCE_FREEZE = "69117b628fe85026e0638abb73dcfbfb04e4a64c"
...
TAPE = ROOT / "configs" / "drtp_b3_development_tape.json"
...
def training_config(arm: str, seed: int, output: Path):
    if arm not in ARMS or seed not in SEEDS:
        raise ValueError("B3 permits only UTR/DRTP × seeds 2701–2703")
    # The strict runner is used only as a frozen configuration template.  No
    # historical checkpoint, seed, budget, or result is reused.
...
def run_one(arm: str, seed: int, output_root: Path) -> None:
    ...
    assert cfg.seed == seed and cfg.drtp_sampler_seed == seed
    manifest = {
        ...
        "seed_replacement": False, "strict_continuation_authorized": False,
        "failure_aware_telemetry": True, "telemetry_window": {"pre": 20, "post": 60},
        "tape_hash": tape["tape_hash"], "tape_path": str(TAPE), "config": cfg.__dict__,
    ...
    try:
        ...
        if missing:
            raise RuntimeError("B3 persistence failure: " + "; ".join(missing))
