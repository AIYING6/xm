...
printf '%s\n' "${SEEDS[@]}" | xargs -r -n1 -P "$MAX_PARALLEL" -I{} bash -lc 'run "$0" scripts/run_sr_drtp_p1.py official --seed "$1" --output-root "$2" --execute > "$2/launcher_logs/official_seed$1.out" 2> "$2/launcher_logs/official_seed$1.err"' "$PYTHON_BIN" {} "$OUTPUT_ROOT"
