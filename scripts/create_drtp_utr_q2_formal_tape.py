"""Create the prospective DRTP/UTR Q2 formal paired evaluation tape."""
...
PROTOCOL = "DRTP-UTR-Q2-FORMAL-PAIRED-TAPE-V1"
TAPE_START, EPISODES = 490000, 100
...
FORBIDDEN_NAMESPACES = tuple(
    ...
    + [f"{start}000-{start}099" for start in range(380, 490, 10)]
)
...
def main() -> None:
    ...
    if not args.execute:
        raise SystemExit("NO-GO: formal tape creation requires explicit --execute")
    manifest = frozen_manifest()
    args.output_root.mkdir(parents=True, exist_ok=True)
    path = args.output_root / "formal_tape_manifest.json"
    if path.exists():
        existing = json.loads(path.read_text(encoding="utf-8"))
        if existing.get("tape_hash") != manifest["tape_hash"]:
            raise RuntimeError("existing formal tape differs from frozen payload")
    else:
