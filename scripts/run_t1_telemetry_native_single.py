"""Run one strictly continuous, from-scratch T1 UTR-SG reference trajectory."""
...
PROTOCOL = "T1-TELEMETRY-NATIVE-UTR-SG-TRAINING-V1"
...
def run_one(seed: int, output_root: Path) -> dict:
    ...
    out_dir = output_root / "runs" / "utr_sg" / f"seed{seed}"
    if out_dir.exists() and any(out_dir.iterdir()):
        raise FileExistsError(f"refusing to overwrite T1 trajectory: {out_dir}")
    out_dir.mkdir(parents=True, exist_ok=False)
    cfg = training_config(seed, out_dir)
    manifest = {
        "protocol": PROTOCOL, "status": "running", "method": "UTR-SG-MAPPO", "arm": "utr_sg", "seed": seed,
        "parameter_count": 116728, "graph_encoder": "single", "actor_gradient_mode": "utr",
        ...
        "tape_hash": tape["tape_hash"], "tape_start": TAPE_START, "config_hash": config_hash(cfg), "config": cfg.__dict__,
    }
    ...
    if missing:
        raise FileNotFoundError(f"incomplete T1 trajectory: {missing}")
