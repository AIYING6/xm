...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the 3DOF no-task-support ablation through matched baseline, topology curriculum, and node-failure evaluation."
    )
    ...
    parser.add_argument("--baseline-dir", type=Path, default=ROOT / "results" / "intercept_3d_no_task_support_baseline_formal")
    parser.add_argument("--out-dir", type=Path, default=ROOT / "results" / "intercept_3d_no_task_support_topology_formal")
...
def write_manifest(args: argparse.Namespace) -> None:
    ...
    lines = [
        "# 3DOF Task-Support Ablation Protocol",
        "",
        ...
        "",
        "```text",
        "Train and evaluate a no-task-support ablation for the multi-relation role graph.",
        "The ablation removes the dynamic task-support relation while keeping perception, communication, role-pair gates, and the union residual path.",
        "```",
        "",
        "## Configuration",
        "",
        ...
        "",
        "```text",
        "This protocol is designed as the formal task-support ablation for the current 3DOF relay/scout-failure claim.",
        "It still needs paired analysis against the full multi-relation checkpoints before being used in manuscript text.",
        "```",
        "",
    ]
