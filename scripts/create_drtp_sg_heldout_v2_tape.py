"""Generate the solely authorized held-out DRTP v2 tape after explicit launch approval."""
...
PROTOCOL = "DRTP-SG-HELDOUT-CONFIRMATION-V2-TAPE-V1"
TAPE_START, EPISODES = 430000, 100
CONDITIONS = {
    "nominal": None,
    "f0_seen_44_80": (44, 80),
...
FORBIDDEN_NAMESPACES = (
    ...
    "380000-380099", "410000-410099", "420000-420099",
)
...
def frozen_manifest() -> dict:
    payload = {
        ...
        "development_namespace_excluded": "420000-420099",
        "held_out_training_seeds": [2001, 2002, 2003],
...
def main() -> None:
    ...
    if not args.execute:
        raise SystemExit("NO-GO: held-out tape creation requires explicit --execute")
    manifest = frozen_manifest()
    args.output_root.mkdir(parents=True, exist_ok=True)
    path = args.output_root / "heldout_tape_manifest.json"
    if path.exists():
        existing = json.loads(path.read_text(encoding="utf-8"))
        if existing.get("tape_hash") != manifest["tape_hash"]:
            raise RuntimeError("existing held-out tape differs from frozen v2 payload")
    else:
