...
def endpoint(agent: C1PilotMAPPO, arm: str, seed: int, settings: Settings) -> tuple[list[dict[str, Any]], dict[str, float]]:
    ...
    with (out / "train_log.csv").open("w", newline="", encoding="utf-8") as handle:
        ...
        for update in range(1, settings.updates + 1):
            ...
            if update % 32 == 0 or update == settings.updates:
                _, validation = endpoint(agent, arm, 93_000 + seed + update, Settings(endpoint_episodes=64))
...
def main() -> None:
    ...
    agent, arm = load_agent(args.checkpoint); rows, summary = endpoint(agent, arm, args.seed, Settings())
