"""Fixed-endpoint, development-only evaluation for DRTP Stabilization V1."""
...
from run_drtp_stabilization_development_v1_single import ARMS, SEEDS, STEPS, UPDATES  # noqa: E402


PROTOCOL = "DRTP-STABILIZATION-DEVELOPMENT-V1-FIXED-ENDPOINT-EVALUATION"
...
def main() -> None:
    ...
    if tape.get("protocol") != "DRTP-STABILIZATION-DEVELOPMENT-V1-TAPE" or tape.get("training_access") != "forbidden":
        raise RuntimeError("invalid V1 development tape")
    tasks, manifests = [], []
    for arm in ARMS:
        for seed in SEEDS:
            ...
            if not (manifest.get("status") == "completed" and manifest.get("updates") == UPDATES
                    and manifest.get("environment_steps") == STEPS and manifest.get("from_scratch") is True
                    and manifest.get("checkpoint_promotion") is False and manifest.get("tape_hash") == tape["tape_hash"]):
                raise RuntimeError(f"invalid source run: {arm}/seed{seed}")
    ...
    print(f"DRTP Stabilization V1 endpoint evaluation: cells={len(tasks)}, episodes={total}, workers={min(args.workers, len(tasks))}", flush=True)
    raw: list[dict] = []
    completed = 0
    with ProcessPoolExecutor(max_workers=min(args.workers, len(tasks)), mp_context=mp.get_context("spawn")) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(f"DRTP Stabilization V1 evaluation progress {completed}/{total} ({100 * completed / total:.2f}%)", flush=True)
    condition_order = {entry["name"]: index for index, entry in enumerate(tape["conditions"])}
        ...
        for seed in SEEDS:
            for condition in condition_order:
                ...
                summary.append({
                    ...
                    "constraint_violation": mean(subset, "constraint_violation"), "control_effort": mean(subset, "control_effort"),
                })
    ...
    manifest = {
        ...
        "endpoint_only": True,
        "checkpoint_selection": "fixed_final_update_only",
        "cells": len(tasks),
        "conditions": [entry["name"] for entry in tape["conditions"]],
        "episodes_per_condition": len(tape["episode_ids"]),
