"""Shared-actor plus non-attacker commit-mask control for L1 attribution."""
...
OUT = ROOT / "results" / "l1_shared_mask_control"
TRAIN_SEEDS = (8401, 8402)
EVAL_SEEDS = tuple(range(840_000, 840_032))
UPDATES = 60
HORIZON = 180
PROTOCOL = "L1_SHARED_ACTOR_WITH_ROLE_ACTION_MASK_CONTROL_V1"
...
def cfg(seed: int, out_dir: Path, updates: int = UPDATES):
    ...
    return replace(
        ...
        role_specific_actor_heads=False,
        ...
        run_id=f"l1_shared_mask_control_seed{seed}",
    )
...
def main() -> None:
    ...
    (OUT / "L1_MANIFEST.json").write_text(json.dumps({
        "status": "L1_SHARED_ACTOR_WITH_ROLE_ACTION_MASK_CONTROL",
        ...
        "only_added_complexity": "shared actor retained; non-attacker engage_commit masked",
    ...
    for seed in TRAIN_SEEDS:
        ...
        trained.append((f"l1_shared_mask_control_seed{seed}", run_cfg, l0.load_agent(run_cfg, ckpt)))
    rows = []
    ...
    learned = [r for r in summary if r["mode"].startswith("l1_shared_mask_control")]
    ...
    if len(positive) == len(TRAIN_SEEDS):
        verdict = "L1_SHARED_MASK_CONTROL_ESTABLISHES_LEARNING_SIGNAL"
    elif len(positive) == 0:
        verdict = "L1_SHARED_MASK_CONTROL_NO_GO__ROLE_SPECIALIZATION_REMAINS_SUPPORTED"
    else:
        verdict = "L1_SHARED_MASK_CONTROL_PARTIAL_UNSTABLE_SIGNAL"
