"""L1 heterogeneous-team, reliable-communication learnability pilot."""
...
OUT = ROOT / "results" / "l1_heterogeneous_reliable_development_v2"
TRAIN_SEEDS = (8201, 8202)
EVAL_SEEDS = tuple(range(820_000, 820_032))
UPDATES = 60
HORIZON = 180
PROTOCOL = "L1_HETEROGENEOUS_TEAM_RELIABLE_COMM_DEVELOPMENT_V1"
...
def main() -> None:
    ...
    (OUT / "L1_MANIFEST.json").write_text(json.dumps({
        "status": "L1_HETEROGENEOUS_TEAM_RELIABLE_COMM_DEVELOPMENT",
        ...
        "only_added_complexity": "Scout+Relay+Attacker with reliable communication",
    ...
    for seed in TRAIN_SEEDS:
        ...
        trained.append((f"l1_heterogeneous_reliable_seed{seed}", run_cfg, l0.load_agent(run_cfg, ckpt)))
    rows = []
    ...
    learned = [r for r in summary if r["mode"].startswith("l1_heterogeneous")]
    ...
    if len(positive) == len(TRAIN_SEEDS):
        verdict = "L1_HETEROGENEOUS_RELIABLE_COMM_LEARNING_SIGNAL_ESTABLISHED"
    elif len(positive) == 0:
        verdict = "L1_HETEROGENEOUS_RELIABLE_COMM_NO_GO__COORDINATION_BOTTLENECK"
    else:
        verdict = "L1_HETEROGENEOUS_RELIABLE_COMM_PARTIAL_UNSTABLE_SIGNAL"
