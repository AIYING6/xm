...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'output'/'DRTP_PLR_EXTERNAL_MATCHED_AB_10M_V2.zip'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
def main():
 ...
 with tempfile.TemporaryDirectory() as x:
  x=Path(x);src=x/'s.zip';stage=x/'DRTP_PLR_EXTERNAL_MATCHED_AB_10M_V2';subprocess.run(['git','archive','--format=zip',f'--output={src}',commit,'--','algorithms','envs','configs/drtp_plr_external_matched_ab_freeze_20260906.json','scripts','requirements.txt','README.md'],cwd=ROOT,check=True);stage.mkdir()
