"""Role-specific actor-head isolation pilot for the L1 coordination bottleneck."""
...
OUT = ROOT / "results" / "l1_role_specific_actor_development_v2"
TRAIN_SEEDS = (8301, 8302)
EVAL_SEEDS = tuple(range(830_000, 830_032))
UPDATES = 60
HORIZON = 180
PROTOCOL = "L1_ROLE_SPECIFIC_ACTOR_HEADS_DEVELOPMENT_V1"
...
def cfg(seed: int, out_dir: Path, updates: int = UPDATES):
    ...
    return replace(
        ...
        run_id=f"l1_heterogeneous_reliable_seed{seed}",
    )
...
def main() -> None:
    ...
    (OUT / "L1_MANIFEST.json").write_text(json.dumps({
        "status": "L1_ROLE_SPECIFIC_ACTOR_HEADS_DEVELOPMENT",
        ...
        "only_added_complexity": "role-specific actor policy heads; non-attacker engage_commit masked",
        "communication_dropout_prob": 0.0, "message_delay_steps": 0, "config": asdict(template),
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    trained = []
    for seed in TRAIN_SEEDS:
        run = OUT / f"l1_heterogeneous_reliable_seed{seed}"
        run_cfg = cfg(seed, run)
        ckpt = run / "actor_critic_latest.pt"
        if not ckpt.exists():
            l0.train_ri_gmappo(run_cfg)
        trained.append((f"l1_role_specific_seed{seed}", run_cfg, l0.load_agent(run_cfg, ckpt)))
    rows = []
    ...
    learned = [r for r in summary if r["mode"].startswith("l1_role_specific")]
    ...
    if len(positive) == len(TRAIN_SEEDS):
        verdict = "L1_ROLE_SPECIFIC_ACTOR_HEADS_ESTABLISH_LEARNING_SIGNAL"
    elif len(positive) == 0:
        verdict = "L1_ROLE_SPECIFIC_ACTOR_HEADS_NO_GO__SHARING_NOT_SUFFICIENT_OR_OTHER_BOTTLENECK"
    else:
        verdict = "L1_ROLE_SPECIFIC_ACTOR_HEADS_PARTIAL_UNSTABLE_SIGNAL"
    payload = {"verdict": verdict, "summary": summary, "performance_use_prohibited": True}
    (OUT / "L1_VERDICT.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
