"""Evaluate every strict-continuous DRTP/UTR learning-curve milestone."""
...
from run_drtp_sg_strict_10m_single import ARMS, MILESTONES, PROTOCOL as TRAIN_PROTOCOL, SEEDS  # noqa: E402


PROTOCOL = "DRTP-SG-STRICT-CONTINUOUS-10M-EVALUATION-V1"
LABELS = tuple(MILESTONES.values())


def checkpoint_for(run_dir: Path, label: str) -> Path:
    return run_dir / "actor_critic_latest.pt" if label == "10m" else run_dir / f"actor_critic_milestone_{label}.pt"
...
def main() -> None:
    ...
    if tape.get("episode_ids") != list(range(420000, 420100)) or tape.get("canonical") is not False:
        raise RuntimeError("invalid frozen 420k development tape")
    eval_root = args.output_root / "evaluations" / "strict_10m"
    ...
    for arm in ARMS:
        for seed in SEEDS:
            ...
            violations = {
                ...
                "no_held_out": manifest.get("held_out_seeds_used") is False,
            }
            if not all(violations.values()):
                raise RuntimeError(f"strict contract violation in {run_dir}: {violations}")
            for label in LABELS:
    ...
    print(f"strict-10M evaluation: workers={workers}, cells={len(tasks)}, episodes={total}", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=workers, mp_context=context) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(f"strict-10M evaluation progress {completed}/{total} ({100 * completed / total:.1f}%)", flush=True)
    condition_order = {item["name"]: index for index, item in enumerate(tape["conditions"])}
    raw_rows.sort(key=lambda row: (row["method"], int(row["train_seed"]), LABELS.index(row["evaluation_budget"]),
    ...
    for row in raw_rows:
        key = (row["method"], int(row["train_seed"]), row["evaluation_budget"], row["topology_condition"])
        ...
        for seed in SEEDS:
            for label in LABELS:
                for condition in condition_order:
                    rows = grouped[(arm, seed, label, condition)]
                    summary.append({
    ...
    manifest = {
        ...
        "tape_start": 420000, "episodes_per_condition": 100, "checkpoint_labels": list(LABELS),
        "raw_rows": len(raw_rows), "cells": len(tasks), "workers": workers,
        "source_runs": source_manifests, "canonical_seeds_used": False, "held_out_tape_used": False,
    }
