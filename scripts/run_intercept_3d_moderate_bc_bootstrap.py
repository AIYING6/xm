...
def main() -> None:
    ...
    device = torch.device(args.device); x = torch.as_tensor(obs.reshape(-1, 3, obs.shape[-1]), device=device); y = torch.as_tensor(actions.reshape(-1, 3), device=device)
    rows: list[dict[str, float | int]] = []
    for epoch in range(1, args.epochs + 1):
        order = torch.randperm(len(x), device=device); losses: list[float] = []; correct = 0; count = 0
        for start in range(0, len(x), args.batch_size):
            index = order[start:start + args.batch_size]; logits = actor_logits(agent, x[index])
        ...
        rows.append({"epoch": epoch, "cross_entropy": float(np.mean(losses)), "action_accuracy": correct / count})
    ...
    manifest = {"protocol": PROTOCOL, "artifact_class": "DEVELOPMENT_ONLY_LOCAL_OBSERVATION_BC_BOOTSTRAP", "paper_evidence": False,
                ...
                "teacher_episodes": args.episodes, "teacher_samples": int(len(obs)), "epochs": args.epochs,
