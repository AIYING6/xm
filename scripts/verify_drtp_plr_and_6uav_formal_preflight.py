...
def main() -> None:
    ...
    parser.add_argument("--line", choices=("plr", "6uav", "both"), default="both")
    ...
    checks = {"training_started": False, "evaluation_started": False}
    freeze_hashes = {}
    if args.line in {"plr", "both"}:
        ...
        freeze_hashes["plr_freeze_sha256"] = _sha(plr_path)
    if args.line in {"6uav", "both"}:
        ...
        freeze_hashes["six_uav_freeze_sha256"] = _sha(six_path)
    if args.line == "both":
        checks["cross_line_seed_ranges_disjoint"] = set(plr["training"]["seeds"]).isdisjoint(six["training"]["seeds"])
