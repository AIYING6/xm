...
def iterable_mean(xs):
 """Preserve the frozen arithmetic while accepting generator expressions."""
 values=list(xs)
 if not values:raise ValueError("frozen gate received an empty metric collection")
...
def main():
 ...
 r=rule();r._mean=iterable_mean;groups={"A":(71011,71012,71013,71014,71015),"B":(71021,71022,71023,71024,71025)};out=a.output_root/"diagnostics"/"egtr_double_cohort_final_gate"
 if out.exists() and any(out.iterdir()):raise FileExistsError(f"refusing to overwrite non-empty gate directory: {out}")
 out.mkdir(parents=True,exist_ok=True);(out/"EGTR_DOUBLE_COHORT_AGGREGATION_REPAIR.md").write_text("# EGTR aggregation repair\n\nThe frozen numeric rule was retained. This runner-only repair materializes iterables before calculating its existing arithmetic mean, allowing the rule's generator-based criteria to execute. It reads only the completed final evaluation; no training, checkpoint, tape, or metric was changed.\n",encoding="utf-8");dec={}
