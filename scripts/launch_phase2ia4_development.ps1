...
foreach ($arm in @(@("full_gate","relation_conditioned"), @("no_role_gate","none"))) {
  foreach ($seed in 101,202,303) {
    ...
    if (Test-Path -LiteralPath $out) { throw "refusing to overwrite existing Phase2IA4 run: $out" }
    $config = Join-Path $configRoot "phase2ia2_$($arm[0]).json"
    $manifest = [ordered]@{ artifact_class="DEVELOPMENT_ONLY"; protocol="PHASE2IA4"; arm=$arm[0]; seed=$seed; git_sha=$gitSha; config_path=$config; config_sha256=(Get-Sha256 $config); updates=3907; environment_steps=1000192; start_time=(Get-Date).ToUniversalTime().ToString("o"); completion_status="running"; checkpoint="actor_critic_latest.pt"; telemetry="role_gate_telemetry.csv"; resume_allowed=$false; early_stopping_allowed=$false; canonical_data_used=$false }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase2IA4 development run failed: $($arm[0]) seed $seed" }
    $checkpoint = Join-Path $out "actor_critic_latest.pt"
    if (!(Test-Path -LiteralPath $checkpoint)) { throw "missing final checkpoint: $checkpoint" }
