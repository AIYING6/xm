"""Build the compact, source-hashed D5 AutoDL execution package."""
...
def selected_files() -> list[Path]:
    ...
    selected.append(ROOT / "scripts" / "launch_drtp_stable_v2_d5_autodl.sh")
    selected.extend([
        ROOT / "configs" / "drtp_stable_v2_d5_pilot_tape.json",
        ROOT / "configs" / "drtp_stable_v2_d5_pilot_freeze.json",
        ROOT / "tests" / "__init__.py",
        ROOT / "tests" / "test_drtp_stable_v2_kl_backtrack.py",
        ROOT / "tests" / "test_drtp_stable_v2_kl_guard.py",
        ROOT / "tests" / "test_drtp_stable_v2_d5_contract.py",
        ROOT / "tests" / "test_tc_sam.py",
        ROOT / "tests" / "test_drtp_utr_q2_formal.py",
    ])
    selected.extend((ROOT / "docs" / "drtp_stable_v2_d4_20260829").glob("*"))
    selected.extend((ROOT / "docs" / "drtp_stable_v2_d5_20260829").glob("*"))
...
def main() -> None:
    ...
    output = (args.output or ROOT / f"DRTP_STABLE_V2_D5_PILOT_05M_{source_commit[:8]}.zip").resolve()
    if output.exists():
    ...
    tape = json.loads((ROOT / "configs" / "drtp_stable_v2_d5_pilot_tape.json").read_text(encoding="utf-8"))
    manifest = {
        "protocol": "DRTP-STABLE-V2-D5-CLOUD-PACKAGE-V1",
        "source_commit": source_commit,
        "algorithm_freeze_commit": "7ebab8d4",
