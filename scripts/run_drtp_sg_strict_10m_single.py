"""Run one authorized strict-continuous DRTP/UTR 0→10M trajectory."""
...
PROTOCOL = "DRTP-SG-STRICT-CONTINUOUS-10M-DEVELOPMENT-V1"
SEEDS = (1901, 1902)
ARMS = {"utr_sg": "utr", "drtp_sg": "drtp"}
NUM_ENVS, ROLLOUT_STEPS, UPDATES = 4, 64, 39063
MILESTONES = {
    1953: "500k", 3907: "1m", 5859: "1500k", 7813: "2m", 9766: "2500k",
    11719: "3m", 13672: "3500k", 15625: "4m", 17578: "4500k", 19531: "5m",
    21484: "5500k", 23438: "6m", 25391: "6500k", 27344: "7m", 29297: "7500k",
    31250: "8m", 33203: "8500k", 35156: "9m", 37109: "9500k", 39063: "10m",
}
...
def training_config(arm: str, seed: int, out_dir: Path) -> RIGMAPPOConfig:
    if arm not in ARMS or seed not in SEEDS:
        raise ValueError("unauthorized strict-10M arm or seed")
...
def verify_config(cfg: RIGMAPPOConfig) -> dict:
    runtime_fields = {
        "model", "optimizer", "global_update", "python_rng", "numpy_rng", "torch_cpu_rng",
        "cuda_rng", "environment_runtime_rng", "current_observation", "episode_counters",
        "sampler_state", "drtp_q_ema_difficulty_window", "normalization_state",
    }
    return {
        "protocol": PROTOCOL,
        ...
        "strict_continuous_from_scratch": True,
        "runtime_state_from_update_zero": cfg.runtime_state_checkpointing,
        "runtime_state_resume": cfg.runtime_state_resume,
        "legacy_resume": cfg.resume,
        "milestones": cfg.milestone_updates,
        "runtime_checkpoint_fields": sorted(runtime_fields),
        "no_held_out_or_canonical": True,
    }
...
def run_one(arm: str, seed: int, output_root: Path) -> dict:
    ...
    manifest = {
        ...
        "held_out_seeds_used": False, "parameter_count": 116728, "graph_encoder": "single",
    ...
    if not final_checkpoint.exists() or not final_runtime.exists():
        raise FileNotFoundError("missing strict-10M final checkpoint or runtime state")
    milestone_hashes, runtime_hashes = {}, {}
    for label in MILESTONES.values():
        ...
        milestone_hashes[label] = sha256(model)
...
def main() -> None:
    ...
    if args.verify_config:
        probe = training_config("utr_sg", 1901, args.output_root / "config_probe")
        print(json.dumps(verify_config(probe), indent=2, default=str))
        return
