...
本文研究限于冻结的三无人机轻量 3DOF 仿真、预定义的中继故障组和五种子 cohort。它不提供一般分布鲁棒、最坏情况优化或实飞可靠性保证。当前主比较识别的是 DRTP 相对于均匀暴露的增量，尚不单独证明自适应更新优于任何静态非均匀分配，也不证明拓扑语义与自适应更新的因果必要性。修复后的 6-UAV v2 虽已验证故障在 episode 内生效，却因 UTR 全条件满分而未形成可判别的跨规模性能证据；它说明故障注入正确性本身不足以保证评价任务具有足够难度。跨规模、完整析因消融与统一硬件下的运行成本仍须以可判别的固定合同和来源审计补强。
...
'''
    ...
    result_to_conclusion = result_to_conclusion.replace(
        "跨规模、外部优先级比较与运行成本需以各自经过审计的结果补强。",
        "外部优先级比较已用于有限定位；修复后的 6-UAV v2 虽证明故障在 episode 内真实发生，却因 UTR endpoint 饱和而未提供可判别的跨规模性能证据。完整析因消融与运行成本仍需以各自经过审计、且端点具有方法区分度的结果补强。",
    )
