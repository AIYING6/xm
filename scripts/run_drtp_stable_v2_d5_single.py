"""Run one future-authorized DRTP-KLB D5 0.5M cloud trajectory."""
...
PROTOCOL = "DRTP-STABLE-V2-D5-PILOT-STAGE1-V1"
...
ARMS = {
    ...
    "drtp_klb_sg": {"sampler": "drtp", "guard": "post_step_actor_backtrack", "target_kl": 0.02},
}
TAPE = ROOT / "configs" / "drtp_stable_v2_d5_pilot_tape.json"
D4_AUDIT = ROOT / "docs" / "drtp_stable_v2_d4_20260829" / "STABLE_V2_D4_TECHNICAL_AUDIT.json"
...
def training_config(arm: str, seed: int, output: Path):
    if arm not in ARMS or seed not in SEEDS:
        raise ValueError("D5 permits exactly three frozen arms x seeds 3201-3203")
    spec = ARMS[arm]
...
def main() -> None:
    ...
    audit = json.loads(D4_AUDIT.read_text(encoding="utf-8"))
    if audit.get("status") != "D4_TECHNICAL_PASS":
        raise RuntimeError("D4 technical audit is not PASS")
    ...
    manifest = {
        ...
        "d4_audit_sha256": sha256(D4_AUDIT),
        "config": cfg.__dict__,
    ...
    try:
        ...
        if missing:
            raise RuntimeError("D5 persistence failure: " + "; ".join(missing))
        manifest.update({
            "status": "completed",
