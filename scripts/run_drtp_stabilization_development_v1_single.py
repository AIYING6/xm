"""Run one frozen 1M Global-Anchored EGTR development trajectory."""
...
PROTOCOL = "DRTP-STABILIZATION-DEVELOPMENT-V1"
FREEZE = ROOT / "configs" / "drtp_stabilization_development_v1_freeze.json"
SEEDS = (76011, 76012, 76013)
ARMS = {
    "utr_sg": ("utr", None),
    "drtp_sg": ("drtp", None),
    "egtr_sg": ("egtr", None),
    "anchored_egtr_a035_sg": ("anchored_egtr", 0.35),
    "anchored_egtr_a055_sg": ("anchored_egtr", 0.55),
    "anchored_egtr_a075_sg": ("anchored_egtr", 0.75),
}
UPDATES, NUM_ENVS, ROLLOUT = 3907, 4, 64
...
def load_tape(output_root: Path) -> dict:
    ...
    if tape.get("protocol") != "DRTP-STABILIZATION-DEVELOPMENT-V1-TAPE":
        raise RuntimeError("unexpected development tape protocol")
    if tape.get("episode_ids") != list(range(760000, 760100)) or tape.get("training_access") != "forbidden":
        raise RuntimeError("invalid frozen V1 development tape")
    return tape
...
def training_config(arm: str, seed: int, out_dir: Path) -> RIGMAPPOConfig:
    ...
    return RIGMAPPOConfig(
        env_name="3d_intercept", seed=seed, num_envs=NUM_ENVS, rollout_steps=ROLLOUT,
        ...
        target_kl=None, save_interval=UPDATES, save_snapshots=False, milestone_updates=MILESTONES,
...
def run_one(arm: str, seed: int, output_root: Path) -> dict:
    if arm not in ARMS or seed not in SEEDS:
        raise ValueError("unfrozen V1 arm or seed")
    ...
    manifest = {
        ...
        "early_stopping": False, "checkpoint_promotion": False, "evaluation_during_training": False,
        "fixed_final_budget_only": True, "tape_hash": tape["tape_hash"], "tape_not_read_by_training": True,
        "freeze_sha256": digest(FREEZE), "source_commit": source_commit(), "config": cfg.__dict__,
    }
    ...
    if missing := [str(path) for path in required if not path.is_file()]:
        raise FileNotFoundError("missing V1 artifacts: " + ", ".join(missing))
    manifest.update({
